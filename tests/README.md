...
| `checks.js` | One browser check per Phase 5 fix, per applied decision and per later fix (30 of them). `node tests/checks.js [name…]`. |
...
node tests/checks.js                       # the 30 per-fix checks
